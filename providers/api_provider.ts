...
const serialize = Object.assign(
  async function (this: HttpContext, ...[data, resolver]: Parameters<ApiSerializer['serialize']>) {
    const serialized = await serializer.serialize(data, resolver ?? this.containerResolver)
    // BaseSerializer treats a plain array as an already serialized collection.
    // Wrap it explicitly so every public API response keeps the documented { data } shape.
    return Array.isArray(serialized) ? { data: serialized } : serialized
  },
  {
    withoutWrapping(
      this: HttpContext,
